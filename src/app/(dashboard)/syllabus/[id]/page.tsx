...
export default function SyllabusDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return (
      <div>
        ...
        <div className="p-6 text-center py-12 text-gray-500">Loading...</div>
      </div>
    );
  }
  ...
  if (!syllabus) {
    return (
      <div>
        <Header title="Syllabus" />
        <div className="p-6 text-center py-12 text-gray-500">Syllabus not found</div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <button onClick={() => router.push("/syllabus")} className="text-blue-600 hover:text-blue-800 text-sm mb-2 inline-block">
          ...
          <div>
            ...
            {syllabus.chapters.length === 0 ? (
              <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
                ...
                <p className="text-gray-400 text-sm mt-1">Add chapters manually or use Bulk Import.</p>
              </div>
            ) : (
                    ...
                    <div key={ch.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                      <div className="flex items-center justify-between px-5 py-3 cursor-pointer hover:bg-gray-50"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-gray-900">{ch.order}. {ch.name}</span>
                          <span className="text-xs text-gray-400">{ch.subtopics.length} subtopics</span>
                          {chHours > 0 && (
                            <span className="text-xs text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded">{chHours}h</span>
                          )}
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-gray-500">{chPercent}%</span>
                          </div>
                          <button onClick={(e) => { e.stopPropagation(); openEditChapter(ch); }}
                            className="text-gray-400 hover:text-blue-600 text-xs">Edit</button>
                          <button onClick={(e) => { e.stopPropagation(); handleDeleteChapter(ch.id); }}
                            className="text-gray-400 hover:text-red-600 text-xs">Delete</button>
                        </div>
                      </div>
                      ...
                      {isExpanded && (
                        <div className="border-t border-gray-100 px-5 py-3 bg-gray-50">
                          {ch.subtopics.length === 0 ? (
                            <p className="text-gray-400 text-sm py-2">No subtopics yet.</p>
                          ) : (
                              ...
                              {ch.subtopics.map((st) => (
                                <li key={st.id} className="flex items-center justify-between py-1.5 px-2 rounded hover:bg-white">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-sm ${st.progress.length > 0 ? "text-gray-500" : "text-gray-700"}`}>
                                      {st.name}
                                    </span>
        ...
        {tab === "assignments" && (
          <div>
            ...
            {batches.length === 0 ? (
              <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
                <p className="text-gray-500">No batches of type {BATCH_LABELS[syllabus.batchType] || syllabus.batchType} exist.</p>
                <p className="text-gray-400 text-sm mt-1">Create a batch first before assigning chapters.</p>
              </div>
            ) : teachers.length === 0 ? (
              <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
                <p className="text-gray-500">No teachers are qualified for {syllabus.subject.name}.</p>
                <p className="text-gray-400 text-sm mt-1">Add subject expertise to teachers first.</p>
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="text-left px-6 py-3 text-sm font-medium text-gray-500">Chapter</th>
                      <th className="text-left px-6 py-3 text-sm font-medium text-gray-500">Assigned Teacher</th>
                      <th className="text-left px-6 py-3 text-sm font-medium text-gray-500">Batch</th>
                      <th className="text-left px-6 py-3 text-sm font-medium text-gray-500">Subtopics</th>
                      ...
                      ch.assignments.length > 0 ? (
                        ch.assignments.map((a) => (
                          <tr key={a.id} className="border-b border-gray-100 hover:bg-gray-50">
                            <td className="px-6 py-3 font-medium text-gray-900">{ch.order}. {ch.name}</td>
                            <td className="px-6 py-3 text-gray-600">{a.teacher.user.name}</td>
                            <td className="px-6 py-3">
                              <span className="bg-cyan-100 text-cyan-700 px-2 py-0.5 rounded text-xs font-medium">{a.batch.name}</span>
                            </td>
                            <td className="px-6 py-3 text-gray-500 text-sm">{ch.subtopics.length}</td>
                        ...
                        <tr key={ch.id} className="border-b border-gray-100 hover:bg-gray-50">
                          <td className="px-6 py-3 font-medium text-gray-900">{ch.order}. {ch.name}</td>
                          <td className="px-6 py-3"><span className="text-gray-400 italic">Unassigned</span></td>
          ...
          <div>
            {report.length === 0 ? (
              <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
                ...
                <p className="text-gray-400 text-sm mt-1">Assign chapters to teachers and they can start marking subtopics as complete.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {report.map((t) => (
                  <div key={`${t.teacherId}_${t.batchId}`} className="bg-white rounded-xl border border-gray-200 p-5">
                    <div className="flex justify-between items-start mb-1">
                      <h3 className="font-semibold text-gray-900">{t.teacherName}</h3>
                      <span className={`text-lg font-bold ${t.percentComplete === 100 ? "text-green-600" : "text-blue-600"}`}>
                        {t.percentComplete}%
                      </span>
                    </div>
                    <div className="mb-3">
                      <span className="bg-cyan-100 text-cyan-700 px-2 py-0.5 rounded text-xs font-medium">{t.batchName}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="text-gray-500">Chapters: <span className="text-gray-900 font-medium">{t.completedChapters}/{t.assignedChapters}</span></div>
                      <div className="text-gray-500">Subtopics: <span className="text-gray-900 font-medium">{t.completedSubtopics}/{t.totalSubtopics}</span></div>
                    </div>
        ...
        <Modal isOpen={chapterModalOpen} onClose={() => setChapterModalOpen(false)} title={editingChapter ? "Edit Chapter" : "Add Chapter"}>
          <form onSubmit={handleChapterSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="button" onClick={() => setChapterModalOpen(false)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-lg hover:bg-gray-200 transition font-medium">
                Cancel
              </button>
            </div>
          </form>
        ...
        <Modal isOpen={subtopicModalOpen} onClose={() => setSubtopicModalOpen(false)} title="Add Subtopic">
          <form onSubmit={handleSubtopicSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="button" onClick={() => setSubtopicModalOpen(false)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-lg hover:bg-gray-200 transition font-medium">
                Cancel
              </button>
            </div>
          </form>
        ...
        <Modal isOpen={bulkModalOpen} onClose={() => setBulkModalOpen(false)} title="Bulk Import Chapters & Subtopics">
          <div className="space-y-4">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 outline-none text-sm font-mono"
              rows={10}
            ...
            {bulkPreview.length === 0 ? (
              <button onClick={handleBulkPreview}
                className="w-full bg-gray-100 text-gray-700 py-2 rounded-lg hover:bg-gray-200 transition font-medium text-sm">
                Preview
              </button>
            ) : (
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2">Preview ({bulkPreview.length} chapters):</h4>
                <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-3 bg-gray-50 text-sm space-y-2">
                  {bulkPreview.map((ch, i) => (
                    <div key={i}>
                      <div className="font-medium text-gray-900">{i + 1}. {ch.name}</div>
                      {ch.subtopics.map((st, j) => (
                        <div key={j} className="ml-4 text-gray-600">- {st}</div>
                      ))}
                    </div>
                  ))}
                </div>
        ...
        <Modal isOpen={assignModalOpen} onClose={() => setAssignModalOpen(false)} title="Assign Chapters to Teacher">
          <form onSubmit={handleAssign} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Select Chapters *</label>
              <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-lg p-2 space-y-1">
                {syllabus.chapters.map((ch) => (
                  <label key={ch.id} className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-50 cursor-pointer">
                    ...
                    <span className="text-sm text-gray-700">{ch.order}. {ch.name}</span>
                    {ch.assignments.length > 0 && (
                      <span className="text-xs text-gray-400">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="button" onClick={() => setAssignModalOpen(false)} className="flex-1 bg-gray-100 text-gray-700 py-2 rounded-lg hover:bg-gray-200 transition font-medium">
                Cancel
              </button>
            </div>
          </form>
