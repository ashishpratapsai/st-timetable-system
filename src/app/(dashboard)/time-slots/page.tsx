...
export default function TimeSlotsPage() {
  ...
  const [form, setForm] = useState({ startTime: "", endTime: "", label: "", order: "", scope: "all" });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await fetch("/api/time-slots", {
      ...
      body: JSON.stringify({ ...form, order: Number(form.order) || slots.length + 1, scope: form.scope || "all" }),
    });
    setModalOpen(false);
    setForm({ startTime: "", endTime: "", label: "", order: "", scope: "all" });
    fetchSlots();
  }
    ...
    <div>
      ...
      <div className="p-3 sm:p-4 md:p-6 animate-fadeIn">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => { setForm({ startTime: "", endTime: "", label: "", order: String(slots.length + 1), scope: "all" }); setModalOpen(true); }}
                  ...
                  <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                        s.scope === "senior" ? "bg-purple-50 text-purple-700" :
                        s.scope === "junior" ? "bg-teal-50 text-teal-700" :
                        "bg-slate-100 text-slate-600"
                      }`}>
                        {s.scope === "senior" ? "Senior" : s.scope === "junior" ? "Junior" : "All"}
                      </span>
                    </td>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select value={form.scope} onChange={(e) => setForm({ ...form, scope: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200">
